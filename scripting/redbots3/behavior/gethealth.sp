...
public Action CTFBotGetHealth_OnStart(BehaviorAction action, int actor, BehaviorAction priorAction, ActionResult result)
{
	...
	{
		return action.Done("Not eligible for health (must be RED team)");
	}
	
	// VERIFICAÇÃO: Deve ser um robô (Defender OU Comprado)
	if (!g_bIsDefenderBot[actor] && !g_bBuyIsPurchasedRobot[actor])
	{
		return action.Done("Not eligible for health (must be Defender OR Purchased robot)");
	}
	
	// ===== ROBÔS GIGANTES NÃO PODEM PEGAR HEALTH =====
	char clientName[64];
	GetClientName(actor, clientName, sizeof(clientName));
	if (StrContains(clientName, "Giant") != -1 || StrContains(clientName, "Boss") != -1)
	{
		return action.Done("Giant robots cannot use health");
	}
	// ==============================================
	...
	ComputeHealthAndAmmoVectors(actor, health, max_range);
	...
	{
		JSONObject entity = view_as<JSONObject>(health.Get(i));
...
{
    // CORREÇÃO CRÍTICA: Verificar se o ator ainda é válido
...
public Action CTFBotGetHealth_ShouldHurry(BehaviorAction action, INextBot nextbot, QueryResultType& result)
{
	...
	// ===== ROBÔS GIGANTES NÃO DEVEM USAR HEALTH =====
	char clientName[64];
...
public Action CTFBotGetHealth_ShouldAttack(BehaviorAction action, INextBot nextbot, CKnownEntity knownEntity, QueryResultType& result)
{
	...
	// ===== ROBÔS GIGANTES NÃO DEVEM ATACAR ENQUANTO BUSCAM HEALTH =====
	char clientName[64];
	GetClientName(me, clientName, sizeof(clientName));
	if (StrContains(clientName, "Giant") != -1 || StrContains(clientName, "Boss") != -1)
	{
...
bool CTFBotGetHealth_IsPossible(int actor)
{
	...
	// ===== ROBÔS GIGANTES NÃO PODEM USAR HEALTH =====
	char clientName[64];
	GetClientName(actor, clientName, sizeof(clientName));
	if (StrContains(clientName, "Giant") != -1 || StrContains(clientName, "Boss") != -1)
	{
