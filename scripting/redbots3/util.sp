...
ArrayList g_hEngineerSpots = null;
...
int EconItemCreateNoSpawn(char[] classname, int itemDefIndex, int level, int quality)
{
	...
	if (item != -1)
	{
		...
		if (StrEqual(classname, "tf_weapon_builder", false))
		{
			...
			SetEntProp(item, Prop_Send, "m_aBuildableObjectTypes", isSapper ? 0 : 1, _, 0);
			SetEntProp(item, Prop_Send, "m_aBuildableObjectTypes", isSapper ? 0 : 1, _, 1);
			SetEntProp(item, Prop_Send, "m_aBuildableObjectTypes", isSapper ? 0 : 1, _, 2);
			SetEntProp(item, Prop_Send, "m_aBuildableObjectTypes", isSapper ? 1 : 0, _, 3);
		}
...
void DetonateObjectOfType(int client, TFObjectType iType, TFObjectMode iMode = TFObjectMode_None, bool bIgnoreSapperState = false)
{
	...
	if (hEvent)
	{
		...
		hEvent.SetInt("objecttype", view_as<int>(iType));
...
bool IsSentryBusterRobot(int client)
{
    ...
    if (IsTFBotPlayer(client) && GetTFBotMission(client) == view_as<int>(CTFBot_MISSION_DESTROY_SENTRIES))
        return true;
    
    char model[PLATFORM_MAX_PATH]; 
    GetClientModel(client, model, PLATFORM_MAX_PATH);
    
    if (StrContains(model, "sentry_buster", false) != -1)
        return true;
    
    char name[MAX_NAME_LENGTH];
    GetClientName(client, name, sizeof(name));
    if (StrContains(name, "Sentry Buster", false) != -1)
        return true;
...
bool IsUpgradeStationEnabled(int station)
{
	...
	return view_as<bool>(GetEntData(station, iOffsetIsEnabled, 1));
}
...
CNavArea PickBuildArea(int client, float SentryRange = 1300.0)
{
    ...
    if (TF2_GetClientTeam(client) == TFTeam_Blue)
    {
        return PickBuildAreaBlue(client);
    }
    
    return randomArea;
}

CNavArea PickBuildAreaBlue(int client)
{
    int iAreaCount = TheNavAreas.Count;
    ...
    ArrayList BlueSpawnExits = new ArrayList();
    ...
    for (int i = 0; i < iAreaCount; i++)
    {
        ...
        {
            BlueSpawnExits.Push(bestConnection);
        }
    }
    
    if (BlueSpawnExits.Length <= 0)
    {
        BlueSpawnExits.Close();
        return PickRandomAreaBlue(client);
    }
    
    CNavArea RandomBlueSpawnExit = BlueSpawnExits.Get(GetRandomInt(0, BlueSpawnExits.Length - 1));
    BlueSpawnExits.Close();
    
    float vecExitCenter[3];
    RandomBlueSpawnExit.GetCenter(vecExitCenter);
    vecExitCenter[2] += 45.0;
    
    ArrayList ValidNestAreas = new ArrayList();
    ...
    for (int i = 0; i < iAreaCount; i++)
    {
        ...
        float flDistanceToExit = GetVectorDistance(center, vecExitCenter);
        
        if (flDistanceToExit > 200.0 && flDistanceToExit < 1000.0)
        {
            ValidNestAreas.Push(area);
        }
    }
    
    CNavArea bestArea = NULL_AREA;
    
    if (ValidNestAreas.Length > 0)
    {
        bestArea = ValidNestAreas.Get(GetRandomInt(0, ValidNestAreas.Length - 1));
    }
    else
    {
        bestArea = PickRandomAreaBlue(client);
    }
    
    ValidNestAreas.Close();
    
    return bestArea;
}

CNavArea PickRandomAreaBlue(int client)
{
    int iAreaCount = TheNavAreas.Count;
    ...
    for (int i = 0; i < iAreaCount; i++)
    {
        ...
        if (IsPathToVectorPossible(client, center))
        {
    ...
    {
        bestArea = ValidAreas.Get(GetRandomInt(0, ValidAreas.Length - 1));
    }
...
CNavArea PickBuildAreaPreRound(int client, float SentryRange = 1300.0)
{
    ...
    {
        CNavArea customSpot = GetRandomEngineerSpot();
        if (customSpot != NULL_AREA)
        {
            if (redbots_manager_debug.BoolValue)
                PrintToChatAll("[Engineer] %N using custom nest spot", client);
            return customSpot;
        }
    }
	...
	ArrayList AreasCloser                  = new ArrayList();
	ArrayList VisibleAreas                 = new ArrayList();
	ArrayList VisibleAreasAfterSentryRange = new ArrayList();
...
void InitializeEngineerSpotSystem()
{
    if (g_hEngineerSpots == null)
    {
        g_hEngineerSpots = new ArrayList(3);
    }
    
    LoadEngineerSpots();
}

void LoadEngineerSpots()
{
    if (g_hEngineerSpots == null)
    {
        g_hEngineerSpots = new ArrayList(3);
    ...
    if (kv.JumpToKey("EngineerSpot"))
    {
        if (kv.GotoFirstSubKey(false))
        {
            do
            {
                ...
                g_hEngineerSpots.PushArray(pos);
...
void SaveEngineerSpots()
{
    if (g_hEngineerSpots == null || g_hEngineerSpots.Length == 0)
    {
        return;
    }
    ...
    kv.JumpToKey("EngineerSpot", true);
    
    float pos[3];
    for (int i = 0; i < g_hEngineerSpots.Length; i++)
    {
        g_hEngineerSpots.GetArray(i, pos);
...
void AddEngineerSpot(float position[3])
{
    if (g_hEngineerSpots == null)
    {
        g_hEngineerSpots = new ArrayList(3);
    ...
    g_hEngineerSpots.PushArray(position);
    SaveEngineerSpots();
}

int GetEngineerSpotCount()
{
    if (g_hEngineerSpots == null)
        return 0;
    return g_hEngineerSpots.Length;
}

bool GetEngineerSpotPosition(int index, float pos[3])
{
    if (g_hEngineerSpots == null || index < 0 || index >= g_hEngineerSpots.Length)
        return false;
    
    g_hEngineerSpots.GetArray(index, pos);
    return true;
}

CNavArea GetRandomEngineerSpot()
{
    if (g_hEngineerSpots == null || g_hEngineerSpots.Length == 0)
    {
        return NULL_AREA;
    ...
    float pos[3];
    int randomIndex = GetRandomInt(0, g_hEngineerSpots.Length - 1);
    g_hEngineerSpots.GetArray(randomIndex, pos);
    
    return TheNavMesh.GetNearestNavArea(pos, true, 500.0, true, true, view_as<int>(TFTeam_Red));
}
